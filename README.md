...
- Automatic checkpoint selection can translate Base prompts for Ref2VA while preserving the original source for recovery. The translation uses the selected reference files and verified continuation media. Authored Ref2VA text needs its original Base source to move to Base. Automatic translation with video/audio references currently requires Linux; use a manual checkpoint on other platforms or for experimental source-audio modes.
...
The Vast.ai option is not a payment processor, a dollar-to-credit exchange, or a promise of compute or service. An owner may record an already-known one-time Vast.ai sponsorship before the $1,000 goal is reached, but the record is attestation-only: it is excluded from development-cost recovery, supporter tiers and implemented benefits, queue priority, and promotional Maestro credits. A refund or chargeback entry is a manual adjustment to an existing owner-attested record only. Maestro does not detect Vast.ai payments, initiate or suggest refunds, create provider credentials, or turn a Vast.ai record into hosted queue allowance. The public Vast.ai destination remains locked until the separate recovery total from eligible sources reaches the goal.

Other signed provider events or owner-verified records may grant the benefits Maestro currently implements: supporter recognition, bounded hosted-queue priority, and promotional Maestro credits under the server-owned benefit policy. Promotional Maestro credits are a hosted queue allowance, never purchased compute, transferable value, a service guarantee, or expanded authority. Buy Me a Coffee remains the existing shared creator page; no second creator account is introduced.

Promotional Maestro credits are never a paywall. Fully funded work can receive bounded priority, while zero, partial, refunded, or expired allowance still creates an otherwise-valid durable job in the lowest ordinary queue band with starvation-bounded capacity. The sealed owner account and local/authenticated-LAN execution are exempt. Tracked defaults keep enforcement off and the execution realm `local`; an operator who deliberately activates hosted scheduling sets `MAESTRO_HOSTED_CREDIT_ENFORCEMENT_ENABLED=true` together with `MAESTRO_COMPUTE_EXECUTION_REALM=hosted`, restarts Maestro, and verifies all three queue bands and the starvation bound. Account or support activation by itself never changes those runtime values.
...
Optional account-data locations can be set in Pinokio's per-app **Configure** tab:
`MAESTRO_ACCOUNT_STORE_PATH`, `MAESTRO_ACCOUNT_PROJECT_MIGRATION_PATH`, and
`MAESTRO_ACCOUNT_PROJECT_MEMBERSHIP_PATH`. Start rereads these values on every
invocation; explicit per-app values override global values, including an empty
value to use the backend default. Relative paths resolve under `app/`. Changing
a path selects a location; it does not copy existing account or project state.
