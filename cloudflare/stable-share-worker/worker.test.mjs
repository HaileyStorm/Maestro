...
test("offline browser navigation gets static no-tracking HTML", async () => {
  ...
  assert.match(html, /Start Maestro from Pinokio on the studio computer/)
  assert.match(html, /<a class="retry" href="">Try again<\/a>/)
  assert.doesNotMatch(html, /No tracking, sign-in|private connection online/)
...
test("restart status uses neutral headings for every valid state and reason", async () => {
  ...
  for (const [index, [state, heading]] of Object.entries(headings).entries()) {
    ...
    const html = await response.text()
    assert.match(html, /<a class="retry" href="">Try again<\/a>/)
    assert.doesNotMatch(html, /This page contains no scripts, tracking/)
