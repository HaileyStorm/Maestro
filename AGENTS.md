...
3. **Example Lock-in:** Identify the closest matching script in `/media/hailey/TVBox/pinokio/prototype/system/examples`. Record its path and keep it open while editing. Every launcher change must mirror that reference unless the user explicitly instructs otherwise.
4. **Pre-flight Checklist:** Convert the applicable rules from this document and `PINOKIO.md` at `/media/hailey/TVBox/pinokio/prototype/PINOKIO.md` into a task-specific checklist (install/start/reset/update structure, regex patterns, menu defaults, log checks, destination path, etc.). Confirm each item is ticked **before** making changes.
...
- Read `docs/operations/FRESH_THREAD_HANDOFF.md` and
  `docs/operations/CONTINUATION.md` before continuing account activation,
  project migration, credits, restart, or release work. They are durable
  cross-session handoffs; live runtime state must still be verified.
- Resolve the repository root dynamically and require both `AGENTS.md` and the
  repo-root `.beads/` tracker before acting. Never encode this machine's checkout
  path in tracked instructions or handoffs.
- At the first checkpoint after a fresh start, run the shared read-only
  activation audit before any Beads lifecycle command: record the exact root,
...
- Use the shared `working_sentinel.py` tool for new structured reservations.
  ...
  writer per file or symbol cluster.
- Preserve every pre-existing change. Inventory dirty paths before edits; never
  use reset, checkout, clean, or an implicit stash to erase or hide unrelated
  work. Stage only explicit owned paths.
- Treat source inspection, mocked tests, local runtime health, live access-surface
  verification, and human acceptance as distinct evidence levels. Report only
  the level actually observed.
- For a coordinated restart, use `app/scripts/restart_status.py` to set and then
  show a bounded public status under one untracked generation value. After the
  intended service and access surface are healthy, clear that exact generation
...
### Launch Maestro Continuum

- Start Continuum through Pinokio `start.js`, not `python wgp.py` or other
  WanGP CLI commands. Classic UI is a separate local-only `start_classic.js`.
- If the Pinokio control plane is down, start the installed Pinokio desktop
  app through the user systemd unit (`systemctl --user start pinokio`), not
  as an agent-owned shell. Agent wrappers impose a runtime limit and kill
  the AppImage plus every Maestro child when that limit fires. A Linux
  AppImage SUID sandbox abort is a runtime Electron flag (`--no-sandbox`),
  not a Maestro file change.
- Resolve the live app with `pterm search`, then `pterm status` and
  `pterm run --default start.js`. Prefer the user systemd unit
  `maestro-continuum` so Start is not an agent-owned shell. Coordinated
  bounce is **Restart Maestro** (`restart.js`). Starting an already-ready
  app is not a restart.
- RTX 50 with driver 580 or newer prefers `app/env-rtx50` when that install
  marker exists. Otherwise Start uses the preserved `app/env` compatibility
  runtime and logs that Update still needs to finish the CUDA 13 migration.
  Driver older than 580 still stops until the operator updates the driver.
- `start.js` sets `SERVER_PORT` with `{{port}}` on the `launch.py` step, not
  before Blender. `launch.py` binds that port before importing torch/WanGP so
  Pinokio's Caddy reverse proxy cannot occupy it during model import. Caddy is
  not part of Blender.
- Operator `MAESTRO_STRICT_SERVER_PORT=true` refuses to hop if that port is
  busy. Set it false only as a temporary recovery; `start.js` still
  re-registers the live share URL after a successful bind.
- Discover the current `ready_url`. Probe `/health` then `/ready`. Never reuse
  a port from a previous session or handoff.

### Current owner and migration assumptions

- Until the owner explicitly changes this assumption, treat local, LAN, and
  Cloudflare access as belonging to one human owner. Accounts, credits, hosted
  metering, and donation-derived scheduling remain disabled or hard-off until
  an explicit activation wave with its own live acceptance.
- Before account enforcement is enabled, take a complete census of every
  pre-existing project. Associate each valid project with the first owner, or
  put an unresolved item into an explicit recoverable quarantine/removal flow.
  Never silently leave a project anonymous, ownerless, hidden, or inaccessible.
  Destructive removal still requires explicit target resolution and approval.
- The agent has compute priority on this computer and may preempt queued
  Continuum generation work when necessary. It may restart Maestro or Pinokio
  for an authorized release or recovery wave, but it must not restart the
  computer. Use the coordinated public restart-status protocol above.
- Write visible UI copy for ordinary users. Administrator-only text may assume
  more project knowledge, but should still explain the action and consequence
  plainly. Keep internal state names, raw backend values, proofs, ledgers,
  agent notes, and similar diagnostics out of primary copy; expose genuinely
  useful raw data only under optional `Technical details`.

### Bird-in-the-hand delivery priority

- Prefer the smallest verified user-visible milestone over expanding dormant
...
- For genuinely multi-file or cross-layer work, choose the smallest useful set
  ...
  writer per file or symbol cluster plus proportionate independent review and
  verification. Do not fan out trivial work, and stop parallel scouting when
  findings converge.
- Serialize owner creation, credentials/recovery-code handling, restarts,
  project migration, and Git/index/ref operations. Never ask an agent to invent
  owner credentials or handle recovery codes on the user's behalf.
...
- When the user dumps links, papers, model cards, attention kernels, tunes,
  workflows, or a research tab set, follow `docs/operations/INTAKE.md`.
- A dump is not a build list. Deduplicate, filter, triage, merge, prioritize,
  extract invariants, read comments/issues, and decide adopt / adapt /
  experiment / benchmark-lead / watch / defer / reject against the shipped
  baseline. Do not implement the list as-is or import upstream graphs and
  frameworks wholesale.
- Write or refresh a dated intake note, then stop unless the user asked to
  implement a decided slice. Do not install, download, start GPU work, or
  change defaults during intake.
...
- Maestro Continuum performs inference and content processing on the user's host. Access through LAN, Cloudflare, or another authorized remote transport does not change that locality.
- Do not add, restore, or retain any Maestro-side content moderation, prompt scanning, content classification, keyword/age/relationship heuristics, safety rewriting, blocking, refusal, or policy-warning layer for locally processed prompts, plans, dialogue, media, or outputs.
- Do not route locally processed content to a third party for moderation or classification.
- Model/provider-owned behavior is not an authorization to add a Maestro moderation layer. Prefer local models, and keep any explicitly selected non-local provider subject to its own separately disclosed terms and privacy gate.
- Preserve non-content safeguards: project/session authorization, consent and privacy state, provider/locality selection, filesystem and path confinement, input shape/type validation, resource limits, malware/security controls, recovery/finality invariants, and redacted error handling. These controls must not inspect or judge creative subject matter.
- Treat moderation implementations, scanner-specific tests/fixtures, refusal envelopes, scanner telemetry, and moderation documentation as obsolete. Remove them when encountered unless the user explicitly reverses this rule.
- Regression checks for relevant changes must prove that locally processed adult, violent, controversial, or otherwise sensitive creative content follows the same execution path as other authorized content and is not inspected or refused by Maestro.

### LAN parity

- Authorized LAN-origin users should receive local-equivalent catalog visibility, capabilities, project and output controls, and truthful status whenever technically possible. Implementation effort alone is not a reason to withhold parity.
- Only concrete security, origin, credential, filesystem, or browser/platform boundaries may differ. Do not weaken authentication, privacy, or local-only mutation gates; make every difference explicit and provide an actionable local path.

### Narrow exception: local response-refusal recovery

- Maestro may provide an opt-in, local-only response-quality retry helper that examines only text already generated by the selected local model for positive, user-configured refusal/evasion wording.
- This helper is the opposite of content moderation: it must not receive or inspect the user prompt, messages, media, requested subject matter, or any content label, and it must not infer refusal from omissions, tone, brevity, vagueness, euphemism, or semantic similarity.
- Matching must use a bounded literal or explicitly configured regular-expression corpus. Ambiguous output, no positive match, malformed patterns, and helper errors fail open and preserve the generated result.
- A positive match may trigger a bounded visible retry; it must not block, hide, rewrite, classify, warn about, or persist the response. Do not send prompts or outputs to a model, VLM, third party, or remote service for this purpose.
- Keep the helper disabled until a user-configured or explicitly owner-approved refusal corpus is present. A versioned default corpus may contain only the owner's positive refusal/evasion examples and conservative high-confidence variants; it must never grow into a subject-matter, policy, or semantic classifier.
...
- Make sure to keep this entire document and `PINOKIO.md` at `/media/hailey/TVBox/pinokio/prototype/PINOKIO.md` in memory with high priority before making any decision. Pinokio is a system that makes it easy to write launchers through scripting by providing various cross-platform APIs, so whenever possible you should prioritize using Pinokio API over lower level APIs.
- When writing pinokio scripts, ALWAYS check the examples folder at `/media/hailey/TVBox/pinokio/prototype/system/examples` to see if there are existing example scripts you can imitate, instead of assuming syntax.
- When implementing pinokio script APIs and you cannot infer the syntax just based on the examples, always search the API documentation `PINOKIO.md` at `/media/hailey/TVBox/pinokio/prototype/PINOKIO.md` to use the correct syntax instead of assuming the syntax.
...
IMPORTANT: ALWAYS try to follow the best practices in the examples folder (`/media/hailey/TVBox/pinokio/prototype/system/examples`) instead of trying to come up with your own structure. The examples have been optimized for the best user experience.
...
- If starting from scratch, first determine what type of project you will be building, and then check the examples folder (`/media/hailey/TVBox/pinokio/prototype/system/examples`) to see if you can adopt them instead of coming up everything from scratch.
...
1. Check the examples in the `/media/hailey/TVBox/pinokio/prototype/system/examples` folder
2. Read `/media/hailey/TVBox/pinokio/prototype/PINOKIO.md` for further documentation on the full syntax
...
- When building launchers for existing projects cloned from a repository, try to stay away from modifying the dynamically resolved project root, even if installations are failing. Instead, try to work around it by creating additional files in the launcher folder, and using those files IN ADDITION to the default project.
...
- The `pinokio.js` launcher script can change dynamically depending on the current state of the script execution. Which means, depending on what the file returns, it can determine what the sidebar looks like at any given moment of the script cycle.
  ...
  - Check the examples in the `/media/hailey/TVBox/pinokio/prototype/system/examples` folder to see how these are being used.
...
- In most cases, the `README.md` file at the dynamically resolved project root contains the instructions needed to install and run the app, but if not, figure it out by scanning the rest of the project files.
...
- `icon`: It's best if we have a user friendly icon to represent the app, so try to get an image and link it from `pinokio.json`.
  - If the repository at the dynamically resolved project root points to GitHub (for example `https://github.com/<USERNAME>/<REPO_NAME>`), ask the user if they want to download the icon from GitHub, and if approved, get the `avatar_url` by fetching `https://api.github.com/users/<USERNAME>`, then download the image to the root folder as `icon.png` and set `icon.png` as the `icon` field of `pinokio.json`.
### 13. Gitignore
