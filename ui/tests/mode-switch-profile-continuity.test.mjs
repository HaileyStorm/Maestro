...
test('mode round-trip restores the complete profile UI envelope and fences late timing metadata', async () => {
  ...
  await withFreshStore(async input => {
    ...
    assert.equal(saved.model_type, 'test_video_model')
...
test('Director uses only a bound technical snapshot and preserves stale mode content', async () => {
  const preset = {
    id: 'director-preset-a',
    name: 'Director A',
    mode: 'video',
    model_type: 'test_video_model',
    profile_version: 2,
    params: {
      ...
      custom_settings: { h3_spectrum_profile: 'cinematic' },
    },
    ui_settings: { resolutionPreset: '1080p', aspectRatio: '9:16' },
    activated_loras: [],
    loras_multipliers: '',
    lora_weights: {},
    spatial_upsampling: '',
  ...
  const optionsA = directorModelOptions('test_video_model', {
    default_guidance_scale: 1,
  })
  const optionsB = directorModelOptions('test_video_model_b', {
    default_num_inference_steps: 11,
    default_guidance_scale: 2.25,
  })

  await withFreshStore(async input => {
    const url = String(input)
    if (url.includes('/api/v1/presets?')) return jsonResponse({ presets: [preset] })
    if (url.endsWith('/api/v1/model-options/test_video_model')) return jsonResponse(optionsA)
    if (url.endsWith('/api/v1/model-options/test_video_model_b')) return jsonResponse(optionsB)
    if (url.endsWith('/api/v1/model-options/test_image_model')) return jsonResponse(modelOptions('test_image_model'))
    if (url.endsWith('/api/v1/defaults/test_video_model')) {
      return jsonResponse({ num_inference_steps: 8, guidance_scale: 1 })
    }
    if (url.endsWith('/api/v1/defaults/test_video_model_b')) {
      return jsonResponse({ num_inference_steps: 11, guidance_scale: 2.25 })
    }
    if (url.includes('/api/v1/loras/')) return jsonResponse({ loras: [], guidance_max_phases: 1 })
    throw new Error(`Unexpected Director snapshot request: ${url}`)
  }, async useStore => {
    installTwoModes(useStore)
    useStore.setState(state => ({
      models: [
        ...state.models,
        { model_type: 'test_video_model_b', family: 'video' },
      ],
      params: {
        ...state.params,
        image_refs: ['/job-only/reference.png'],
        video_guide: '/job-only/guide.mp4',
        negative_prompt: 'preserve this content field',
      },
      durationSeconds: 27,
      spatialUpsampling: 'flashvsr',
      resolutionPreset: '1080p',
    ...
    // setParam is the canonical in-session snapshot producer.
    useStore.getState().setParam('guidance_scale', 6.75)
    useStore.getState().setParam('custom_settings', {
      h3_spectrum_profile: '',
      h3_sol_dense_steps: 0,
      h3_turbo_profile: false,
      image_ref_keyword_content: 'creative custom payload',
    })
    assert.equal(useStore.getState().savedParamsPerMode.video.model_type, 'test_video_model')

    // A mode round-trip exercises the same producer through setGenerationMode.
    useStore.getState().setGenerationMode('image')
    useStore.getState().setGenerationMode('video')
    assert.equal(useStore.getState().params.model_type, 'test_video_model')
    assert.equal(useStore.getState().params.guidance_scale, 6.75)

    // loadPreset is another canonical producer and must retain the binding.
    await useStore.getState().loadPresets()
    const loadedPreset = useStore.getState().presets[0]
    assert.ok(loadedPreset)
    assert.equal(await useStore.getState().loadPreset(loadedPreset), true)
    assert.equal(useStore.getState().savedParamsPerMode.video.model_type, 'test_video_model')

    // Reapply the technical values and keep job-local fields in the snapshot.
    useStore.setState(state => ({
      params: {
        ...state.params,
        image_refs: ['/job-only/reference.png'],
    ...
    // Keep the in-memory job fields clean while the saved snapshot still
    // contains them; Director owns its current scene/reference inputs.
    useStore.setState(state => ({
      params: { ...state.params, image_refs: [], video_guide: '' },
    }))

    // Simulate a persisted selection that moved this mode to model B while
    // its last technical snapshot still belongs to model A. Mode-owned UI and
    // job content remain available, but A's technical values are fenced out.
    useStore.setState(state => ({
      generationMode: 'image',
      selectedModelPerMode: { ...state.selectedModelPerMode, video: 'test_video_model_b' },
      params: { ...state.params, model_type: 'test_image_model' },
    }))
    useStore.getState().setGenerationMode('video')
    await new Promise(resolve => setTimeout(resolve, 10))
    const mismatchedMode = useStore.getState()
    assert.equal(mismatchedMode.params.model_type, 'test_video_model_b')
    assert.equal(mismatchedMode.params.guidance_scale, 2.25)
    assert.notDeepEqual(mismatchedMode.params.custom_settings, {
      h3_spectrum_profile: '',
      h3_sol_dense_steps: 0,
      h3_turbo_profile: false,
    })
    assert.equal(mismatchedMode.params.custom_settings.image_ref_keyword_content, 'creative custom payload')
    assert.deepEqual(mismatchedMode.params.image_refs, ['/job-only/reference.png'])
    assert.equal(mismatchedMode.params.video_guide, '/job-only/guide.mp4')
    assert.equal(mismatchedMode.resolutionPreset, '1080p')
    assert.equal(mismatchedMode.aspectRatio, '9:16')
    assert.equal(mismatchedMode.spatialUpsampling, 'flashvsr')

    // Return to model A for the bound Director request. The A snapshot is
    // still intact, and the current Director job inputs are independently
    // cleared before the request boundary below.
    await useStore.getState().selectModel('test_video_model')
    await new Promise(resolve => setTimeout(resolve, 10))
    useStore.setState(state => ({
      params: { ...state.params, image_refs: [], video_guide: '', custom_settings: undefined },
    }))

    const starts = []
    useStore.setState({
      startGeneration: async () => {
        starts.push(structuredClone(useStore.getState().params))
      },
      directorAnalysis: { duration: 2 },
      directorClipPlans: [{ video_prompt: 'director shot', image_prompt: '' }],
      directorPlannedClips: [{ start: 0, end: 2, duration_frames: 48 }],
      directorResolution: '720p',
      directorAspectRatio: '16:9',
      directorResolutionModelType: 'test_video_model',
      directorResolutionOptions: optionsA,
      directorVideoInferenceStepsByModel: {},
      directorClipImages: [],
    })
    // Director must retain supported false/zero/empty technical values while
    // omitting the snapshot's model marker, UI envelope, and job media.
    useStore.getState().directorGenerate()
    await new Promise(resolve => setTimeout(resolve, 250))
    const sameModelParams = starts.at(-1)
    assert.equal(sameModelParams.guidance_scale, 6.75)
    assert.deepEqual(sameModelParams.custom_settings, {
      h3_spectrum_profile: '',
      h3_sol_dense_steps: 0,
      h3_turbo_profile: false,
    })
    assert.deepEqual(sameModelParams.image_refs, [])
    assert.equal(sameModelParams.video_guide, '')
    assert.equal(Object.hasOwn(sameModelParams, 'uiSettings'), false)

    // Selecting model B leaves the A snapshot in place by design. Its
    // technical values must be rejected, while content/media remain intact.
    useStore.setState(state => ({
      params: { ...state.params, custom_settings: undefined },
    }))
    await useStore.getState().selectModel('test_video_model_b')
    await new Promise(resolve => setTimeout(resolve, 10))
    useStore.setState({
      directorResolutionModelType: 'test_video_model_b',
      directorResolutionOptions: optionsB,
    })
    useStore.getState().directorGenerate()
    await new Promise(resolve => setTimeout(resolve, 250))
    const staleModelParams = starts.at(-1)
    assert.equal(useStore.getState().savedParamsPerMode.video.model_type, 'test_video_model')
    assert.equal(staleModelParams.model_type, 'test_video_model_b')
    assert.equal(staleModelParams.guidance_scale, 2.25)
    assert.equal(staleModelParams.custom_settings, undefined)
    assert.deepEqual(staleModelParams.image_refs, [])
    assert.equal(staleModelParams.video_guide, '')
  })
})
