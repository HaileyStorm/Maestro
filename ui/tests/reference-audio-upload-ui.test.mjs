...
test('InputsPanel and TTS audio source contracts use mounted pickers and audio upload', async () => {
  ...
  assert.match(inputsSource, /executeSemanticMediaAdmission\(semanticMediaAdmissionRef/)
  assert.match(inputsSource, /if \(!operations\.isCurrent\(\)\) return 'stale'[\s\S]*operations\.measure\(\)/)
  assert.match(inputsSource, /current\.generationMode === generationMode/)
  assert.match(inputsSource, /current\.durationSeconds === durationSeconds/)
  assert.match(inputsSource, /current\.params === params/)
  assert.match(inputsSource, /current\.modelOptions === modelOptions/)
  assert.match(inputsSource, /current\.imageRefs === imageRefs/)
  assert.match(inputsSource, /currentSemanticRefDurations\.current === semanticRefDurations/)
  assert.match(inputsSource, /semanticMediaAdmissionBusy !== null/)
  assert.match(inputsSource, /busy=\{semanticMediaAdmissionBusy === 'video'\}/)
  assert.match(inputsSource, /busy=\{semanticMediaAdmissionBusy === 'audio'\}/)
  assert.match(inputsSource, /setParam\('video_prompt_type', `\$\{vpt\}KFI`\)/)
  assert.match(inputsSource, /vpt\.replace\(\/KFI\/g, ''\)/)
