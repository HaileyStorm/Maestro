...
test('model selection, terms, and manual-install actions retain authority and exact URLs', () => {
  ...
  assert.match(inputs, /MiniMax H3 ·/)
  const h3InputDetailsMatch = inputs.match(/<summary className="[^"]+">Input details<\/summary>/)
  assert.ok(h3InputDetailsMatch, 'found MiniMax H3 input details disclosure')
  const h3InputDetails = h3InputDetailsMatch[0]
  assertMobileTarget(h3InputDetails, 'MiniMax H3 input details disclosure')
  assert.match(h3InputDetails, /focus-visible:outline-none/)
  assert.match(h3InputDetails, /focus-visible:ring-2/)
  assert.match(h3InputDetails, /focus-visible:ring-accent-blue/)
  assert.match(inputs, /!h3TermsAccepted && `\$\{HOST_TERM_NOTICES\.minimax_h3_ref2va\.text\} Notice v\$\{HOST_TERM_NOTICES\.minimax_h3_ref2va\.version\}\. `/)
  assert.match(inputs, /h3TermsAccepted \? ' · Accepted' : '\.'/)
  assert.doesNotMatch(inputs, /MiniMax H3 Ref2VA model terms are accepted for this Maestro installation/)
  assert.match(inputs, /Remaining: \{h3InputCompatibility\.remaining\.images\} images/)
  assert.match(inputs, /h3InputCompatibility\.semanticReason/)
  assert.match(inputs, /h3InputCompatibility\.frameReason/)
  assert.match(inputs, /h3InputCompatibility\.audioReason/)
  assert.match(inputs, /const supportsInject = useMemo\(\(\) => \{\s*\/\/[\s\S]*if \(h3StudioWorkflow\) return false/)
  assert.match(inputs, /const addInjectFrame = async[\s\S]*?if \(h3StudioWorkflow\) return/)
  assert.match(inputs, /if \(!h3StudioWorkflow\) \{\s*injectedFrames\.forEach/)
  assert.match(inputs, /resolveRestoredSemanticImageRemoval\(/)
  assert.match(inputs, /current\.findIndex\(frame => frame\.path === path\)/)
  assert.match(inputs, /current\.filter\(\(_, index\) => index !== localIndex\)/)
  assert.match(inputs, /supportsInject,\s*\n\s*\},/)
  assert.match(inputs, /semanticVideoDurationStatus !== 'ready'/)
  assert.match(inputs, /semanticAudioDurationStatus !== 'ready'/)
  assert.match(inputs, /Checking saved reference durations/)
  assert.match(inputs, /duration unavailable/)
  assert.match(inputs, /onClear=\{\(\) => removeSemanticVideo\(index\)\}/)
  assert.match(inputs, /onClear=\{\(\) => removeSemanticAudio\(index\)\}/)
  assert.match(inputs, /onClear=\{\(\) => \{[\s\S]*setParam\('image_refs', removal\.imageRefs\)/)
  assert.doesNotMatch(inputs, /Automatic mode uses MiniMax H3 FL2VA and Ref2VA/)
