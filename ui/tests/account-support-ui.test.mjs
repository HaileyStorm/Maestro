...
test('account drawer keeps secrets ephemeral and uses the shared accessible modal contract', async () => {
  ...
  assert.match(source, /priority: required \? 200 : 100/)
  assert.ok(source.includes("required ? 'z-[200]' : 'z-[170]'"))
  assert.match(source, /required \? \(\s*<div aria-hidden="true" className="absolute inset-0 bg-black\/70 pointer-events-none" \/>/)
  assert.match(source, /className="relative z-10 flex h-full min-h-0 w-full flex-col overflow-hidden/)
