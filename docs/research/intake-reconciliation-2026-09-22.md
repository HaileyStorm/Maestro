...
| 2.2–2.3 YuE2 / My Music / training / instrumental / multi-LoRA | The current bridge exposes a project-filtered My Music library, guide-backed instrumental composition, and native multi-LoRA selection through the installed Sound/Vision service. No-LoRA, single-LoRA, combined-LoRA, manually scored instrumental, adapter-led automatic-score, and guide-backed instrumental takes have live technical receipts. The combined take reached a generation limit. The revised guide writer now has live signed-in drafting and clean no-LoRA score-to-audio evidence, but missed the requested bar count. The dedicated score-first LoRA reached the semantic cap with that supplied score while an identical-score, same-seed no-LoRA control ended cleanly; the UI warns and offers adapter-led score planning or base-model supplied-score rendering. One pair and metadata cannot establish listening quality. Training, dataset/checkpoint audition, and owner listening remain open. Adapt the upstream workflow to the existing fast-storage AI Toolkit trainer and joint-v9 tokenizer pair rather than importing a duplicate YuE2 runtime or copying base weights onto NVMe; require reviewed datasets, project ownership, GPU leases, interruption recovery, and selected-checkpoint listening before activation. |
