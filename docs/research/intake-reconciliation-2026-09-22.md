...
| Javawock H3 Bridge workflow | Adopt native executable slice | Native project-bound Gallery selection now binds two source revisions and sealed frame ranges, generates a Ref2VA AddGuide interval, and assembles an A + bridge + B final without importing the source graph. Two signed-in stable-share submissions completed 345- and 328-frame H.264/AAC results under separate exact GPU grants; see [GPU acceptance](../operations/GPU_ACCEPTANCE.md). The second run verified backend Queue visibility and Gallery publication, and exposed a client store-reconnect fault that now has a focused fix. Immediate Queue visibility after that client fix still needs live acceptance. Sampled first-run seam frames retained the subject. Full playback, soundtrack quality, and owner acceptance remain open. |
