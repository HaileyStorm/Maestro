...
| Multishot, Extender, Context Loop and continuation suites | Adapt | Native segment planners/continuity/recovery exist. Bridge and one-still FL2VA Guide now have separate native executors; broader Guide media and native continuation remain separate. No graph import. |
...
| H3 Bridge, Guide, Control and Regenerate 2K | Adapt / experiment | Bridge has an executor and Gallery action. A separate Gallery action now binds one authorized still to an interior frame of an H3 Base FL2VA clip. A signed-in stable-share submission completed one 124-frame A/V output under a validated GPU grant; Queue, Gallery, sidecar hash, full decode, and sampled source-frame similarity passed. The current in-app browser displayed a black HEVC picture while its timeline advanced; an H.264 control played visibly, so browser playback for HEVC remains open. Guide video/audio, multiple guide images, Control, native continuation, and Regenerate 2K remain separate; `h3_control_plan.py`, `h3_native_continuation.py`, and `h3_regenerate_2k.py` are still plan/admission only. See [GPU acceptance](../operations/GPU_ACCEPTANCE.md). |
...
3. The queued single-source Editor export now has a live render, signed-in Gallery receipt and one signed-in concurrent-write race with a single saved winner; complete owner playback acceptance and broaden the concurrency/recovery checks, then expand
