...
| 2.0 Editor / layered timeline / export | The first Continuum Editor slice opens a project Gallery video into a non-destructive, single-source trim timeline and autosaves its draft with revision/CAS and current-source checks. Its preview is bound to a content hash and current server-owned privacy flag; the Gallery action also covers videos without generation metadata. After a September 25 coordinated restart, local and stable-share `/health` and `/ready` each returned 200; in the signed-in stable-share UI, a 5.2-second private video opened in Editor, its preview played, and a 5.0-second trim draft survived Gallery exit/reopen. The CPU export follow-on renders one saved source cut as a separate H.264/AAC MP4, copies all audio streams by re-encoding from timeline zero, retains project privacy and provenance, and queues through the normal cancellation and restart-recovery path. Focused route/media checks cover a nonzero trim start, source replacement, privacy, cancellation, failure cleanup, and restart descriptor sealing; the full backend gate passed 5,352 tests (17 skipped), syntax and JSON grammar checks. The UI gate passed 720 tests, type-check/build, targeted lint, and four synthetic desktop/mobile browser cases. Coordinated service restart and signed-in live export remain pending for that follow-on. Layered editing and AI round trips remain future work; upstream cross-folder browsing is not authorization. Owner acceptance and a live concurrent-write stress test remain open. |
...
3. Finish the single-source Editor export gate and live acceptance, then expand
