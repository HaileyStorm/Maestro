...
| H3 output verification (`source-092`) | Adopt the verification contract, not the source shell script | The August 23 decision calls for structured checks of frame grid, duration, dimensions, rate, promised audio, black/frozen samples and audible signal. The historical note says those checks reached the benchmark runner, not ordinary delivery. Verify the current delivery path and add the missing CPU media-integrity checks before claiming this adopted contract is complete; do not inspect creative subject matter. |
