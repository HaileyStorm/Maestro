...
| YuE2 and installed Sound/Vision LoRAs | Adopt | Bridge-based generation/composition and a project My Music playback/download library are delivered. Native baseline, DreamPop v2, combined artist+style, manually scored instrumental, and optional score-first instrumental-adapter takes have live technical receipts. A refreshed LoRA catalog blocks generation if a previously selected adapter has disappeared, instead of silently dropping it. The dedicated instrumental adapter was pinned, converted without tensor-value changes and exercised against a same-seed no-LoRA control; one pair does not establish musical superiority. It remains an explicit selection, with automatic selection on the Instrumental toggle unimplemented. Instrumental composition now selects arrangement/score guides instead of lyric-writing or vocal-direction guides, asks for `[Instrumental]` without sung words, and retains YuE2's required two-voice ABC score; that writer change is CPU-checked but not yet live-drafted. The combined take reached the 360-second semantic limit and needs ending/listening review. The local training starter has a CPU-only dataset/asset preflight, neutral trigger placeholders and the required portable FFmpeg 8 environment; a disposable valid-audio fixture passed, while missing caption, wrong trigger and invalid audio failed before model load. Maestro training UI, durable job control, real training, checkpoint auditions, and owner listening remain separate work. |
...
| 2.2–2.3 YuE2 / My Music / training / instrumental / multi-LoRA | The current bridge exposes a project-filtered My Music library, guide-backed instrumental composition, and native multi-LoRA selection through the installed Sound/Vision service. No-LoRA, single-LoRA, combined-LoRA, manually scored instrumental, and dedicated score-first instrumental-adapter takes have live technical receipts. The combined take reached a generation limit; the revised instrumental guide writer still has only CPU/static evidence. A same-seed instrumental adapter/control pair executed, but one pair and metadata cannot establish listening quality. Training, dataset/checkpoint audition, and owner listening remain open. Adapt the upstream workflow to the existing fast-storage AI Toolkit trainer and joint-v9 tokenizer pair rather than importing a duplicate YuE2 runtime or copying base weights onto NVMe; require reviewed datasets, project ownership, GPU leases, interruption recovery, and selected-checkpoint listening before activation. |
