...
| 2.2–2.3 YuE2 / My Music / training / instrumental / multi-LoRA | The current bridge already exposes a project-filtered My Music library, guide-backed instrumental composition, and native multi-LoRA selection through the installed Sound/Vision service. One no-LoRA and one single-LoRA take have live technical receipts; multi-LoRA combinations and instrumental audio still need live acceptance. Training and dataset/checkpoint audition remain open. Adapt the upstream workflow to the existing fast-storage AI Toolkit trainer and joint-v9 tokenizer pair rather than importing a duplicate YuE2 runtime or copying base weights onto NVMe; require reviewed datasets, project ownership, GPU leases, interruption recovery, and selected-checkpoint listening before activation. |
