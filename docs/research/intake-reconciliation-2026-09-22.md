...
| 2.2–2.3 YuE2 / My Music / training / instrumental / multi-LoRA | The current bridge exposes a project-filtered My Music library, guide-backed instrumental composition, and native multi-LoRA selection through the installed Sound/Vision service. No-LoRA, single-LoRA, combined-LoRA, manually scored instrumental, adapter-led automatic-score, and guide-backed instrumental takes have live technical receipts. The combined take reached a generation limit. The guide writer now has a signed-in eight-bar-per-voice draft accepted by the installed native parser and a clean no-LoRA score-to-audio take; an earlier draft missed the requested bar count, and the rendered 45-second audio exceeded the later score's 17-second nominal duration. Exact audio-length fidelity remains open. The dedicated score-first LoRA reached the semantic cap with that supplied score while an identical-score, same-seed no-LoRA control ended cleanly; the UI warns and offers adapter-led score planning or base-model supplied-score rendering. One pair and metadata cannot establish listening quality. Training, dataset/checkpoint audition, and owner listening remain open. Adapt the upstream workflow to the existing fast-storage AI Toolkit trainer and joint-v9 tokenizer pair rather than importing a duplicate YuE2 runtime or copying base weights onto NVMe; require reviewed datasets, project ownership, GPU leases, interruption recovery, and selected-checkpoint listening before activation. |
...
For native YuE2 LoRA training, keep Maestro as the project authorization and
control surface while Sound/Vision owns a separate durable training job and
launches the installed fast-storage AI Toolkit. The existing trainer preflight
checks media, captions, the joint-v9 pair, and fast-storage paths, but its
wrapper has no GPU-coordinator lease, its run-name collision is only a warning,
and neither service maps a project's authorized media to a training dataset.
Training must therefore remain unavailable in the UI until project-owned
dataset staging, controlled configurations, generation/training serialization,
fresh bounded GPU grants, cancellation and interrupted-job recovery are in
place. Finished checkpoints need owner audition before any explicit LoRA
catalog install; the current host-wide LoRA scanner must not publish a
project's unreviewed checkpoint automatically. The CLI starter remains a
manual, coordinator-gated path rather than a first-class training feature.
