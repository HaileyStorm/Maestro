...
| --- | --- |
| YuE2 composition, lyrics, ABC and installed LoRA selection | `086ef7d`, `38f4d3e`; `yue2_bridge.py`, `Yue2Controls.tsx`. Bridge/UI and prompt assembly use the existing local Sound/Vision service. September 23 follow-on live checks completed a native YuE2 take and a DreamPop v2 LoRA take, both visible in the project My Music library; exact technical receipts are in [GPU_ACCEPTANCE.md](../operations/GPU_ACCEPTANCE.md). The September 24 training-control update below adds project-bound staging, queue/UI control, and one live 200-step training run; checkpoint auditions and owner listening remain separate work. |
...
This proves one local execution, not a useful adapter or quality. Checkpoint
recovery, selected-checkpoint audition, owner listening, and Windows acceptance
remain open. The former CLI starter remains a separate manual path.
