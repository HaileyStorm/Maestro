...
| CharacterSheet / Krea identity LoRAs | Adapt | `character_sheet_workflow.py`, capabilities and profile gates are planning/validation only; profiles explicitly unavailable/non-executable. Ordinary Reference Packs are a different delivered capability. The corrected contract requires a verified FLUX anchor, gives Quad a face close-up plus front/side/back full-body panels, starts optional visual review Off, and permits user-selected failed-role Qwen Image Edit repair. These are CPU contract changes, not an executor. The first real `quad_flux2_klein` job still needs server-verified anchor provenance, exact base/LoRA and terms binding, durable queue/recovery, and atomic project-asset publication. Do not enable it from static readiness alone. Krea, Dynamic Krea, Triple FLUX, and the separate H3 Orbit Sheet experiment remain outside that first slice. |
