...
class RecipeUiSourceHarnessTests(unittest.TestCase):
    ...
    def test_remote_mobile_navigation_and_overlay_are_reachable_and_bounded(self):
        ...
        self.assertIn("installModalFocus({", sidebar)
        self.assertIn("priority: 60", sidebar)
        self.assertIn("closeModalIfTop(document, mobileSidebarRef.current", sidebar)
        self.assertNotIn("document.addEventListener('keydown'", sidebar)
        self.assertIn("overscroll-contain", sidebar)

        self.assertIn('aria-modal="true"', overlay)
        self.assertIn('z-[100]', overlay)
        self.assertIn("createPortal(", overlay)
        self.assertIn("document.body", overlay)
        self.assertIn("installModalFocus({", overlay)
        self.assertIn("priority: 100", overlay)
        self.assertIn("restoreFocus,", overlay)
        self.assertIn("closeModalIfTop(document, dialogRef.current, closeOverlay)", overlay)
        self.assertNotIn("document.addEventListener('keydown'", overlay)
        self.assertIn("h-[100vh]", overlay)
        self.assertIn("supports-[height:100dvh]:h-[100dvh]", overlay)
        ...
        self.assertIn("min-h-11 min-w-11 shrink-0 items-center justify-center", overlay)
        self.assertIn("flex-1 min-h-0 overflow-y-auto", overlay)
