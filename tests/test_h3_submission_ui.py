...
"""
    ...
    def test_studio_and_director_present_ceiling_and_estimate_semantics(self):
        ...
        for source in (duration, director):
            self.assertIn("Maximum segment length", source)
            self.assertIn("Estimated segments", source)
            self.assertIn("not a target or average", source)
            self.assertIn("shorter, unequal", source)
    ...
    def test_postplan_surfaces_use_authoritative_count_time_and_geometry(self):
        ...
        self.assertIn("Planned segments {plan.clip_count}", dialog)
