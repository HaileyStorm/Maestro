...
from services.h3_legal_access import (  # noqa: E402
    H3LegalAccessError,
    H3_DEPLOYMENT_TERRITORY,
...
from services import h3_legal_access as legal_policy  # noqa: E402
...
def _load_launch_function(source: str, name: str, namespace: dict) -> None:
    ...
    exec(compile(ast.fix_missing_locations(module), "launch-slice", "exec"), namespace)


class H3LegalAccessPolicyTests(unittest.TestCase):
    def test_official_license_binding_and_host_are_frozen(self):
        self.assertEqual(
            H3_UPSTREAM_REVISION,
        ...
        self.assertEqual(H3_DEPLOYMENT_TERRITORY, "US")
        ...
        for model_type in sorted(H3_REGISTERED_MODEL_TYPES):
            with self.subTest(model_type=model_type):
                ...
                with self.assertRaisesRegex(
                    H3LegalAccessError, "separate written MiniMax license",
                ):
    ...
    def test_unknown_and_environment_values_cannot_enable_h3(self):
        ...
        for territory in ("", "unknown", "CA"):
            with self.subTest(territory=territory), patch.object(
                legal_policy, "H3_DEPLOYMENT_TERRITORY", territory,
            ):
                with self.assertRaises(H3LegalAccessError):
                    require_h3_execution_allowed(["minimax_h3"])
...
class H3LegalAccessWiringTests(unittest.TestCase):
    ...
    def test_ui_disables_instead_of_offering_terms_as_unlock(self):
        ...
        self.assertIn("disabled={m.availability_status === 'legal_blocked' || m.execution_allowed === false}", self.system_settings)
        self.assertIn("const legalBlocked = useStore", self.generate_button)
        self.assertIn("? 'License required'", self.generate_button)
        self.assertIn(
        ...
        for source in (
            ...
            self.assertIn("a separate written MiniMax license is required", source)
