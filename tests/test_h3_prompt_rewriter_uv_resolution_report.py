...
def _hashed_requirements(reference: str) -> bytes:
    parents = {name: [] for name, _version, _tag, _deps in PACKAGE_ROWS}
    for parent, _version, _tag, dependencies in PACKAGE_ROWS:
        for dependency in dependencies:
            parents[dependency].append(parent)
    roots = {name for name, _version in closure.ROOT_PACKAGE_PINS}
    lines = [
        "# This file was autogenerated by uv via the following command:",
        "#    uv pip compile <private>",
    ]
    for name, version, tag, _dependencies in PACKAGE_ROWS:
        filename = _wheel_filename(name, version, tag)
        lines.extend(
            [
                f"{name}=={version} \\",
                "    --hash=sha256:" + hashlib.sha256(filename.encode()).hexdigest(),
            ]
        )
        provenance = ([f"-r {reference}"] if name in roots else []) + sorted(
            parents[name]
        )
        if len(provenance) == 1:
            lines.append(f"    # via {provenance[0]}")
        else:
            lines.append("    # via")
            lines.extend(f"    #   {parent}" for parent in provenance)
        index = (
            resolver.PYTORCH_INDEX
            if name in {"torch", "torchvision"} or name.startswith("nvidia-")
            else resolver.PYPI_INDEX
        )
        lines.append(f"    # from {index}")
    return ("\n".join(lines) + "\n").encode("utf-8")
...
class _ProcessFactory:
    ...
    def __call__(self, command, **kwargs):
        ...
        if self.payload is not None:
            payload = (
                _hashed_requirements(str(Path(kwargs["cwd"]) / producer.INPUT_NAME))
                if destination.name == producer.HASHED_REQUIREMENTS_CANDIDATE_NAME
                else self.payload
            )
...
class H3PromptRewriterUvResolutionReportTests(unittest.TestCase):
    ...
    def parse_report(self, payload: bytes | None = None):
        reference = "/private/requirements.in"
        return producer.parse_uv_resolution_evidence_to_wheel_report(
            payload or _pylock(),
            _hashed_requirements(reference),
            requirements_reference=reference,
            size_resolver=lambda *_values: 100,
        )
    ...
    def test_plan_is_path_free_network_free_and_exactly_bound(self):
        ...
        self.assertEqual(first.document["resolver"]["sequential_invocations"], 2)
        self.assertEqual(
            first.document["resolver"]["candidate_output_names"],
            [
                producer.PYLOCK_CANDIDATE_NAME,
                producer.HASHED_REQUIREMENTS_CANDIDATE_NAME,
            ],
        )
    ...
    def test_real_uv_evidence_requires_graph_hash_and_size_sources(self):
        reference = "/private/requirements.in"
        payload = _pylock().replace(b", size = 100,", b",", 1)
        calls = []

        def size_resolver(url, package, index, filename):
            calls.append((url, package, index, filename))
            return 123

        report = producer.parse_uv_resolution_evidence_to_wheel_report(
            payload,
            _hashed_requirements(reference),
            requirements_reference=reference,
            size_resolver=size_resolver,
        )
        self.assertEqual(calls[0][1], "accelerate")
        self.assertEqual(report["packages"][0]["wheel"]["size_bytes"], 123)

        false_marker = _pylock().replace(
            b'version = "1.12.0"\n',
        ...
        with self.assertRaisesRegex(
            ...
            "outside the reviewed target",
        ):
            producer.parse_uv_resolution_evidence_to_wheel_report(
                false_marker,
                _hashed_requirements(reference),
                requirements_reference=reference,
    ...
    def test_missing_pylock_hash_requires_one_exact_generated_hash(self):
        reference = "/private/requirements.in"
        payload = re.sub(
            rb"hashes = \{sha256 = \"[0-9a-f]{64}\"\}",
            b"hashes = {}",
            _pylock(),
            count=1,
        )
        ...
        self.assertEqual(report["packages"][0]["name"], "accelerate")

        requirements = _hashed_requirements(reference).replace(
            b"    # via -r /private/requirements.in",
            b"    --hash=sha256:" + b"f" * 64 + b"\n"
            b"    # via -r /private/requirements.in",
            1,
        )
        with self.assertRaisesRegex(
    ...
    def test_invalid_graph_is_rejected_before_any_size_lookup(self):
        reference = "/private/requirements.in"
        payload = _pylock().replace(b", size = 100,", b",", 1)
        requirements = _hashed_requirements(reference).replace(
            b"    # via -r /private/requirements.in",
            b"    # via\n    #   -r /private/requirements.in\n    #   missing-parent",
            1,
        )
        lookup = mock.Mock(side_effect=AssertionError("size lookup occurred"))
        with self.assertRaisesRegex(
            producer.H3PromptRewriterUvResolutionSecurityError,
    ...
    def test_execution_uses_one_scrubbed_bounded_fake_uv_and_private_outputs(self):
        ...
        for name in (
            ...
            producer.HASHED_REQUIREMENTS_NAME,
